...
export function pruneAutoGeneratedSnapshots(repoRoot: string): void {
  const archDir = path.join(repoRoot, 'architecture');
  if (!fs.existsSync(archDir)) return;
  for (const fname of fs.readdirSync(archDir).filter((f) => f.endsWith('.md'))) {
    const fpath = path.join(archDir, fname);
    try {
      const parsed = matter(fs.readFileSync(fpath, 'utf-8'));
      const isAutoSnapshot =
        parsed.data.name === 'project-architecture-snapshot' ||
        (Array.isArray(parsed.data.tags) &&
          (parsed.data.tags as string[]).includes('auto-generated'));
      if (isAutoSnapshot) fs.unlinkSync(fpath);
    } catch {
...
export async function codeScanCommand(
  ...
  if (options.summarize) {
    pruneAutoGeneratedSnapshots(repoRoot);
