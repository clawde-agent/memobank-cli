...
export interface MemoryTemplate {
  ...
  updated?: string; // ISO 8601 date (auto-set)
  reviewAfter?: string; // e.g., "90d", "6m"
 ...
 */
const SENSITIVE_PATTERNS: { pattern: RegExp; label: string; replacement: string }[] = [
 ...
 */
export function sanitizeContent(content: string): { sanitized: string; redacted: string[] } {
 ...
 */
export function checkSensitiveTopics(content: string): string[] {
  const found: string[] = [];
...
export function validateMemoryContent(content: string): {
  ...
  if (sensitiveTopics.length > 0) {
    warnings.push(
      `Content mentions sensitive topics: ${sensitiveTopics.join(', ')}. Ensure no actual secrets are included.`
    );
  }
  ...
  if (!hasHighLevel && !hasMiddleLevel) {
    warnings.push(
      'Content may be too specific. Consider documenting at a higher abstraction level.'
    );
  }
  ...
  if (filePaths && filePaths.length > 3) {
    warnings.push(
      'Content contains many file paths. Ensure they are relative and not system-specific.'
    );
  }
 ...
 */
export function generateMemorySlug(name: string): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
 ...
 */
export function generateMemoryFile(template: MemoryTemplate): {
  fileName: string;
  content: string;
} {
 ...
 */
export function getTemplateByType(type: MemoryType): string {
 ...
 */
export function checkAbstractionLevel(content: string): 'high' | 'medium' | 'low' | 'too-specific' {
  const lowerContent = content.toLowerCase();
