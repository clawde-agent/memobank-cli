...
function walkPython(tree, source, relPath) {
    const symbols = [];
    const edges = [];
    let currentClass = null;
    function findEnclosingPyFn(node) {
        let cur = node.parent;
        while (cur) {
        ...
        switch (node.type) {
            ...
            case 'call': {
                ...
                if (fnNode.type === 'identifier') {
                    ...
                    if (attr)
                        targetName = getNodeText(attr, source);
                }
                if (targetName) {
                    edges.push({
...
function walkGo(tree, source, relPath) {
    const symbols = [];
    const edges = [];
    function visit(node) {
        switch (node.type) {
...
function walkRust(tree, source, relPath) {
    const symbols = [];
    const edges = [];
...
function walkCSharp(tree, source, relPath) {
    const symbols = [];
    const edges = [];
    let currentClass = null;
    function visit(node) {
