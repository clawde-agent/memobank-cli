...
async function runSetup(state, gitRoot) {
    ...
    if (!existingGitignore.includes('.env')) {
        fs.writeFileSync(gitignorePath, existingGitignore + (existingGitignore.endsWith('\n') ? '' : '\n') + '.env\n', 'utf-8');
    }
...
async function onboardingCommand() {
    ...
    async function fetchModelsForOnboarding(provider, apiKey, baseUrl) {
        const models = await (0, capture_provider_1.fetchAvailableModels)(provider, apiKey, baseUrl);
    ...
    function OnboardingApp() {
        ...
        const [captureModelItems, setCaptureModelItems] = useState([]);
        ...
        return React.createElement(Box, { flexDirection: 'column', padding: 1 }, React.createElement(Text, { bold: true, color: 'cyan' }, '🧠  Memobank Setup'), React.createElement(Text, null, ' '), state.step === 'project-name' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, null, 'Project name:'), React.createElement(TextInput, {
        ...
        })) : null, state.step === 'capture-provider' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { bold: true }, 'Capture LLM provider'), React.createElement(Text, { dimColor: true }, '  Powers AI memory extraction after each session'), React.createElement(SelectInput, {
            items: [
        ...
        })) : null, state.step === 'capture-key' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { bold: true }, `${state.captureProvider} API key`), React.createElement(Text, { dimColor: true }, '  Saved to .memobank/.env (not committed)'), React.createElement(TextInput, {
            ...
            onSubmit: async (value) => {
                ...
                if (!key)
                    return;
                const envKey = state.captureProvider === 'anthropic' ? 'ANTHROPIC_API_KEY' :
        ...
        })) : null, state.step === 'capture-base-url' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { bold: true }, 'Base URL'), React.createElement(Text, { dimColor: true }, state.captureProvider === 'ollama'
            ? '  Ollama endpoint (default: http://localhost:11434/v1)'
            : '  OpenRouter endpoint (default: https://openrouter.ai/api/v1)'), React.createElement(TextInput, {
        ...
        })) : null, state.step === 'capture-model' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { bold: true }, 'Select capture model'), captureModelItems.length > 0
            ...
            : React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { dimColor: true }, '  (type model name and press Enter)'), React.createElement(TextInput, {
                value: '',
                onChange: () => { },
                onSubmit: (value) => {
        ...
        })) : null, state.step === 'embedding-key' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { bold: true }, `${state.embeddingProvider === 'openai' ? 'OpenAI' : 'Jina AI'} API key (for embeddings)`), React.createElement(Text, { dimColor: true }, '  Saved to .memobank/.env — press Enter to skip'), React.createElement(TextInput, {
            ...
            onSelect: (item) => {
                ...
                if (alreadyHaveKey) {
                    if (setupRunning.current)
                        return;
                    setupRunning.current = true;
                    const finalState = { ...state, step: 'done', enableReranker: true, rerankerProvider: provider };
                    setState(finalState);
                    runSetup(finalState, gitRoot).then(({ lines, autoMemoryWarning: warn }) => {
                        setSummary(lines);
                        setAutoMemoryWarning(warn);
        ...
        })) : null, state.step === 'reranker-key' ? React.createElement(Box, { flexDirection: 'column' }, React.createElement(Text, { bold: true }, `${state.rerankerProvider === 'jina' ? 'JINA_API_KEY' : 'COHERE_API_KEY'}:`), React.createElement(Text, { dimColor: true }, '  Saved to .memobank/.env — press Enter to skip'), React.createElement(TextInput, {
