...
class CodeIndex {
    ...
    upsertFile(filePath, language, hash, mtime) {
        this.db
            .prepare(`INSERT INTO files (path, language, hash, mtime)
         VALUES (?, ?, ?, ?)
    ...
    upsertSymbols(filePath, symbols, edges) {
    ...
    linkMemory(memoryPath, description) {
        ...
        const tx = this.db.transaction(() => {
            ...
            const ins = this.db.prepare('INSERT OR IGNORE INTO memory_symbol_refs (memory_path, symbol_hash) VALUES (?, ?)');
            for (const r of results) {
                if (r.symbol.hash)
                    ins.run(memoryPath, r.symbol.hash);
            }
        });
        tx();
    }
    getLinkedMemories(query) {
        const MAX_DEPTH = 2;
        // Wrap in quotes for FTS5 phrase safety; strip existing quotes first
        const ftsQuery = `"${query.replace(/"/g, '')}"`;
        const rows = this.db
            .prepare(`WITH RECURSIVE reachable(hash, depth) AS (
           SELECT s.hash, 0
           FROM symbols_fts
           JOIN symbols s ON symbols_fts.rowid = s.id
    ...
    getStats() {
        ...
        const syms = this.db.prepare('SELECT COUNT(*) AS n FROM symbols').get().n;
        const edges = this.db.prepare('SELECT COUNT(*) AS n FROM edges').get().n;
